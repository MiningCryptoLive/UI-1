...
import {IBalanceChange} from "../Backend/Generated";
...
@Component({
    template: require("./MinerBalanceList.html"),
    components: {
        "list-pager": PagerComponent,
        "spinner": SpinnerComponent,
    },
})
export class MinerBalanceList extends Vue {
    @Lifecycle
    public async created(): Promise<any> {
    }

    public changes = [] as IBalanceChange[];
    public isBusy = false;
    ...
    private async loadCurrentPage() {
        const response = await poolService.listMinerBalanceChanges(modelInit.poolId, modelInit.miner, this.currentPage, this.pageSize);
        this.changes = response.result;
